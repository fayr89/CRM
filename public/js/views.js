...
export async function renderOrders(main) {
  ...
  main.append(
    ...
    isWarehouse
      ? helpBanner('🏭 Склад: заказы в статусе «Зарезервирован» собираются в разделе «Отгрузки». Здесь — общий список заказов для контроля.')
      : helpBanner('💡 Как работать: 1) «Новый заказ» → площадка, клиент (B2C/B2B), способ оплаты и отправки. 2) Для Avito укажите ссылку на диалог и QR-код отправления. 3) Добавьте товары — цена подставится из прайса, при пороге суммы можно дать скидку. 4) Переведите заказ в «Зарезервирован» — он уйдёт складу и попадёт в кассу. Без QR для Avito-доставки зарезервировать нельзя.'),
    toolbar,
    tableArea,
  );
  reload();
...
export async function renderCashbox(main) {
  ...
  main.append(
    ...
    canConfirm
      ? helpBanner('💰 Финансист/админ: проверяйте транзакции (приход по заказам приходит автоматически при резерве). Проставьте комиссию площадки в строке, затем «Подтвердить» или «Отклонить». Баланс = приход − расход − комиссия. Ручные приход/расход — кнопкой «Добавить транзакцию».')
      : helpBanner('💰 Здесь ваши транзакции. Приход по заказу появляется автоматически при переводе в «Зарезервирован» и ждёт подтверждения финансиста. Ручную транзакцию добавьте кнопкой выше.'),
    summaryArea,
...
export async function renderShipping(main) {
  ...
  main.append(
    ...
    canEdit
      ? helpBanner('🏭 Склад: 1) Внизу — заказы к отгрузке (статус «Зарезервирован»). 2) «⬇ Выгрузить в Excel» — файл с QR-кодами и количеством. 3) Отметьте галочками и «Отгрузить выбранные» (или «Отгрузить все») — заказы перейдут в «Отгружен», менеджеры получат уведомление. Колонка QR: «✗ нет!» — Avito-доставка без QR-кода.')
      : null,
  );
