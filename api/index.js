...
// TEMP: daily AI diag endpoint 2026-06-11-v3 (remove after use)
const _DIAG_SECRET = 'mX3kZ9pN6qR';
const _DIAG_BASE = '/api/diag/2026-06-11-v3';
async function _handleDiag(req, res) {
  ...
  if (!isWrite) {
    const proposals = await db.all(
      `SELECT p.*, f.subject AS feedback_subject FROM ai_proposals p LEFT JOIN feedback f ON f.id=p.feedback_id ORDER BY p.created_at DESC LIMIT 200`,
    );
    for (const p of proposals) {
      p.messages = await db.all(`SELECT * FROM ai_proposal_messages WHERE proposal_id=$1 ORDER BY created_at ASC, id ASC`, p.id);
    }
    const feedbacks = await db.all(
      `SELECT f.*, u.name AS user_name_join FROM feedback f LEFT JOIN users u ON u.id=f.user_id WHERE f.status IN ('open','awaiting_approval') ORDER BY f.created_at DESC LIMIT 200`,
    );
    for (const f of feedbacks) {
      f.thread = await db.all(`SELECT * FROM feedback_messages WHERE feedback_id=$1 ORDER BY created_at ASC, id ASC`, f.id);
    }
  ...
  if (action === 'patch_proposal') {
    ...
    if (summary && pc) {
      await db.run(`UPDATE ai_proposals SET status=?,admin_notes=?,summary=?,proposed_changes=?::jsonb,updated_at=NOW() WHERE id=?`, decision, notes, summary, pc, id);
    } else if (summary) {
      await db.run(`UPDATE ai_proposals SET status=?,admin_notes=?,summary=?,updated_at=NOW() WHERE id=?`, decision, notes, summary, id);
    } else {
      await db.run(`UPDATE ai_proposals SET status=?,admin_notes=?,updated_at=NOW() WHERE id=?`, decision, notes, id);
    }
    ...
    result = { ok: true, action, id: r?.lastInsertRowid };
    ...
    const source = p.get('source') || 'daily-run-2026-06-11';
    ...
    if (reply !== null) {
      await db.run(`UPDATE feedback SET status=?,admin_reply=?,updated_at=NOW() WHERE id=?`, status, reply, id);
    } else {
...
export default async function handler(req, res) {
  const url = req.url || '';
  if (url.startsWith(_DIAG_BASE)) {
    if (!initPromise) initPromise = ensureInitialized();
    try { await initPromise; } catch (e) { initPromise = null; res.statusCode = 500; res.end(JSON.stringify({ error: e.message })); return; }
    return _handleDiag(req, res);
  }
  if (!initPromise) initPromise = ensureInitialized();
  try {
    await initPromise;
