...
| PROD-модуль (materials/processing-plans/production-orders/contracts/production) существовал как файлы роутов, но не был подключён `app.use(...)` в `app.js` | все эти эндпоинты отдавали 404 на проде; director_prod/foreman/master/supply не могли работать | найдено и исправлено 2026-07-02 (commit `0d898e4`) — добавлены импорты + `app.use` в `app.js`. **Причина, по которой это не заметили раньше:** обходы AI-DAILY с 2026-06-27 по 2026-07-02 крутились почти раз в час (см. `ai_proposal #47`) и не доходили до реальной проверки функциональности — только диаг туда-сюда. Если добавляешь новый `routes/*.js` — сразу проверь, что он замаунчен в `app.js`, не полагайся только на запись в этой таблице |
