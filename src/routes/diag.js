...
const router = Router();
const SECRET = 'ai-daily-2026-06-11-X7k9';
...
router.get('/act', checkSecret, asyncHandler(async (req, res) => {
  ...
  if (op === 'post_feedback_msg') {
    const row = await db.run(
      `INSERT INTO feedback_messages (feedback_id, user_name, role, text) VALUES (?,?,?,?) RETURNING id, created_at`,
      id, 'AI ассистент', 'admin', msg
    );
    return res.json({ ok: true, op, id, row });
  }
  ...
  if (op === 'create_proposal') {
    ...
    const row = await db.run(
      ...
      data.source || 'daily-run-2026-06-11', JSON.stringify(data.proposed_changes || []),
  ...
  if (op === 'update_proposal_pending') {
    const data = JSON.parse(Buffer.from(json, 'base64').toString('utf8'));
    await db.run(
      `UPDATE ai_proposals SET title=?, summary=?, category=?, risk=?, proposed_changes=?, status='pending', updated_at=NOW() WHERE id=?`,
