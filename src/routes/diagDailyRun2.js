// TEMP: диагностический эндпоинт для ежедневного AI-обхода 2026-06-11 (сессия 2).
...
// Patch proposal status: GET /proposal-patch?secret=X&id=N&status=S&notes=...
router.get(
  '/proposal-patch',
  asyncHandler(async (req, res) => {
    const id = Number(req.query.id);
...
// Create proposal: GET /proposal-create?secret=X&title=...&summary=...&category=...&risk=...&source=...&feedback_id=N
router.get(
  '/proposal-create',
  asyncHandler(async (req, res) => {
...
// Patch feedback status: GET /feedback-patch?secret=X&id=N&status=S&reply=OPTIONAL
router.get(
  '/feedback-patch',
  asyncHandler(async (req, res) => {
    const id = Number(req.query.id);
