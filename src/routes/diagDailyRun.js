// TEMP: диагностический эндпоинт для ежедневного обхода AI (daily-run-2026-06-17-v56).
...
// op=proposal           title=... summary=... category=... risk=... source=... feedback_id=N
...
router.get('/op', asyncHandler(async (req, res) => {
  ...
  if (op === 'proposal-revision') {
    ...
    await db.run(
      `UPDATE ai_proposals SET ${sets}, status = 'pending', updated_at = NOW() WHERE id = ?`,
      ...vals, proposal_id,
    );
    return res.json({ ok: true });
  }
