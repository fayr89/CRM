// TEMP: daily AI run diag endpoint 2026-06-11-v5. Remove after use.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'dr_20260611_v5_7nm4q2';
        ...
        try {
          switch (op.t) {
            ...
            case 'revise_proposal':
              await db.run(
                `UPDATE ai_proposals SET status='pending', title=?, summary=?, updated_at=NOW() WHERE id=?`,
                op.title, op.summary, op.id,
              );
              results.push({ t: op.t, id: op.id, ok: true });
              break;
