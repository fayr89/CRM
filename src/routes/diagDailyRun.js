// ВРЕМЕННЫЙ диаг-эндпоинт для AI daily-run 2026-06-26-v12.
// Снести сразу после использования отдельным коммитом.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'diag-daily-2026-06-26-v12-rT4p';

router.get('/daily-run-v12', async (req, res) => {
...
// POST /api/diag/daily-action-v12?secret=... — выполнить список write-действий
router.post('/daily-action-v12', async (req, res) => {
  if (req.query.secret !== SECRET) return res.status(403).json({ error: 'forbidden' });
  ...
  for (const act of actions) {
    try {
      if (act.type === 'feedback_status') {
        ...
        if (act.summary !== undefined) { sets.push('summary=?'); vals.push(act.summary); }
        if (act.proposed_changes !== undefined) { sets.push('proposed_changes=?::jsonb'); vals.push(JSON.stringify(act.proposed_changes)); }
