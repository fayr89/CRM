...
// Возвращает: ai_proposals (с тредами) по статусам + feedback (open/awaiting_approval) с тредами
router.get(
  '/',
  asyncHandler(async (req, res) => {
    if (!checkSecret(req, res)) return;

    // ai_proposals по всем активным статусам
...
// POST /api/diag/daily-run/feedback/:id/message?token=X — добавить в тред
router.post(
  '/feedback/:id/message',
  asyncHandler(async (req, res) => {
    if (!checkSecret(req, res)) return;
...
// PATCH /api/diag/daily-run/feedback/:id?token=X — сменить статус
router.patch(
  '/feedback/:id',
  asyncHandler(async (req, res) => {
    if (!checkSecret(req, res)) return;
...
// POST /api/diag/daily-run/proposals?token=X — создать ai_proposal
router.post(
  '/proposals',
  asyncHandler(async (req, res) => {
    if (!checkSecret(req, res)) return;
...
// PATCH /api/diag/daily-run/proposals/:id?token=X — решение по предложению
router.patch(
  '/proposals/:id',
  asyncHandler(async (req, res) => {
    if (!checkSecret(req, res)) return;
...
// POST /api/diag/daily-run/proposals/:id/message?token=X — добавить в тред proposal
router.post(
