...
const router = Router();
const SECRET = '7e3b2c9a4f1d8e6b3c2a9f4e1d8b6c3a';
const VERSION = 'v59';
...
// Выполняет одно действие. Параметры зависят от op.
//
// op=post-fb-msg   feedback_id=N  text=...
// op=patch-fb      feedback_id=N  status=...  reply=... (необязательный)
// op=post-proposal-msg  proposal_id=N  text=...
// op=patch-proposal     proposal_id=N  decision=done|approved|rejected|revision  notes=...
// op=create-proposal    feedback_id=N(opt)  title=...  summary=...  category=...
//                        risk=low|medium|high  source=...  proposed_changes=JSON
router.get(
  '/daily-run/act',
  asyncHandler(async (req, res) => {
    if (!checkToken(req, res)) return;
    ...
    // Находим admin-пользователя для записи в user_id
    ...
    if (op === 'create-proposal') {
      ...
      const r = await db.run(
        ...
        fbId, title, summary, category, risk, source,
