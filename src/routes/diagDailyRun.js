...
router.get('/', asyncHandler(async (req, res) => {
  ...
  if (what === 'all' || what === 'proposals') {
    ...
    // Обращения open/awaiting_approval
...
// ?s=SECRET&action=proposal_create&title=T&summary=S&category=C&risk=R&source=SRC&feedback_id=N
...
router.get('/write', asyncHandler(async (req, res) => {
  const { action, id, fid, text, status, reply,
    title, summary, category, risk, source, feedback_id } = req.query;
