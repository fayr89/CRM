...
const SECRET = 'diag-2026-06-02-9dSBO';
const SESSION = 'daily-run-2026-06-02-9dSBO';
const router = Router();
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    // rev-prop — создать новый proposal взамен пересмотренного, старый → done
    if (a === 'rev-prop') {
      const oldId = Number(req.query.old);
      const fid = req.query.fid ? Number(req.query.fid) : null;
      const t = String(req.query.t || '');
      const sum = String(req.query.sum || '');
      const cat = String(req.query.cat || 'feature');
      const risk = String(req.query.risk || 'medium');
      if (!oldId || !t || !sum) return res.status(400).json({ error: 'old, t, sum required' });
      const r = await db.run(
        `INSERT INTO ai_proposals (feedback_id, title, summary, category, risk, source, proposed_changes)
