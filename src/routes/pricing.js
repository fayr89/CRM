...
// ===== Ревизия прайса и ознакомление (без блокировок — только баннер/отчёт) =====

// Статус для текущего пользователя: когда прайс менялся, подтвердил ли он,
// нужно ли ему подтверждение (продающие роли).
router.get(
  '/revision',
  asyncHandler(async (req, res) => {
    ...
    res.json({
      ...
      // показываем баннер, если роль продающая и ещё не ознакомлен с актуальным
      show_banner: priceAckRequiredForRole(req.user.role) && !current,
    });
  }),
);
