// TEMPORARY DIAG — daily AI run v16 — DELETE AFTER USE
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'dr16-xP3mN7kL9vQ';
  ...
  try {
    ...
    if (op === 'feedback') {
      const rows = await db.all(
        `SELECT f.*, u.name AS user_name_author, u.email AS user_email FROM feedback f LEFT JOIN users u ON u.id = f.user_id WHERE f.status IN ('open','awaiting_approval') ORDER BY f.created_at DESC LIMIT 200`,
      );
      const result = [];
