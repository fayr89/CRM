// TEMP: daily-run diag endpoint (сессия o2e4f). Удалить после обхода.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = '2q8PZfEGgn-btVTkgiGfHw3EfvNG1xGQ';
...
// GET /api/diag/daily-o2e4f?secret=...&action=...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    // --- get-feedback (open + awaiting_approval, с thread) ---
    ...
    return res.status(400).json({ error: 'unknown action', available: ['get-proposals','patch-proposal','get-feedback','patch-feedback','post-message','post-proposal'] });
  } catch (err) {
