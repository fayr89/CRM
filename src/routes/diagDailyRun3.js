// TEMP: диагностический эндпоинт для ежедневного AI-обхода 2026-06-11 (сессия 3).
...
// Patch proposal: GET /proposal-patch?secret=X&id=N&status=S&notes=...
router.get(
  '/proposal-patch',
  asyncHandler(async (req, res) => {
    const id = Number(req.query.id);
...
// Patch feedback: GET /feedback-patch?secret=X&id=N&status=S&reply=OPTIONAL
router.get(
  '/feedback-patch',
  asyncHandler(async (req, res) => {
    const id = Number(req.query.id);
