...
// GET /api/diag/ai-daily?secret=SECRET[&action=...&params...]
...
const SECRET = 'daily-run-9zL5wP2mX8vQ';
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!action) {
      ...
      const proposalMessages = proposalIds.length
        ? await db.all(
            `SELECT * FROM ai_proposal_messages WHERE proposal_id IN (${proposalIds.map(() => '?').join(',')}) ORDER BY created_at ASC, id ASC`,
            ...proposalIds,
          )
        : [];
      ...
      const feedbackMessages = feedbackIds.length
        ? await db.all(
            `SELECT * FROM feedback_messages WHERE feedback_id IN (${feedbackIds.map(() => '?').join(',')}) ORDER BY created_at ASC, id ASC`,
            ...feedbackIds,
          )
        : [];
    ...
    if (action === 'patch_fb') {
      ...
      await db.run(
        ...
        newStatus, adminReply, justResolved ? null : cur.resolved_by, feedbackId,
      );
    ...
    if (action === 'post_proposal') {
      ...
      const proposedChanges = req.query.proposed_changes
        ? decodeURIComponent(req.query.proposed_changes)
        : null;
