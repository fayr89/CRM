// TEMP diag endpoint for AI daily run 2026-06-08-v27. Remove after use.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'daily-v27-9xKq2mRp';
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    // ------- WRITE: post proposal thread message -------
    if (op === 'post-proposal-message') {
      ...
      const r = await db.run(
        `INSERT INTO ai_proposal_messages (proposal_id, user_name, role, text)
         VALUES ($1, 'AI ассистент', 'admin', $2) RETURNING id`,
        id,
        text,
      );
      return res.json({ ok: true, data: await db.get('SELECT * FROM ai_proposal_messages WHERE id = $1', r.lastInsertRowid) });
    }
    ...
    res.json({ ok: false, error: 'unknown op', ops: ['get-proposals', 'get-proposal-messages', 'post-proposal-message', 'patch-proposal', 'post-proposal', 'get-feedback', 'post-feedback-msg', 'patch-feedback'] });
  } catch (e) {
