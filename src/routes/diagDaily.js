// TEMPORARY diag endpoint for AI daily run 2026-06-05-v43.
// DELETE this file after use.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'AI-D43-xP9n5q2026';
...
router.get('/act', async (req, res) => {
  ...
  try {
    ...
    action = JSON.parse(Buffer.from(normalized, 'base64').toString('utf8'));
  } catch {
  ...
  try {
    switch (action.type) {
      ...
      case 'proposal-update': {
        // Update summary/proposed_changes for revision
        await db.run(
        ...
        return res.json({ ok: true, action: 'proposal-update', id: action.id });
      }
