// TEMP — ежедневный AI-обход 2026-06-06-v50. Снести сразу после использования.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'daily-2026-06-06-v50-x9q3rk';
...
router.get('/daily-read', async (req, res) => {
  ...
  try {
    ...
    // Все открытые/ожидающие обращения
...
// GET /api/diag/daily-act?secret=...&actions=<base64json> — выполнить список действий
// actions = JSON-массив [{type, ...params}]
// Поддерживаемые типы:
//  {type:'create_proposal', title, summary, category, risk, source, feedback_id, proposed_changes}
//  {type:'patch_proposal', id, decision, notes}
//  {type:'proposal_message', id, text}
//  {type:'patch_feedback', id, status, admin_reply}
//  {type:'feedback_message', id, text}
router.get('/daily-act', async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({ error: 'invalid actions base64/json: ' + e.message });
  }
