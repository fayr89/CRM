// TEMP diag endpoint for AI daily run 2026-06-25-v9 — remove after use
import { Router } from 'express';
import { db } from '../db.js';
import { asyncHandler } from '../errors.js';

const DIAG_SECRET = 'dr9-2026-06-25-x8k2pQ4nR7';
const router = Router();
...
// Full data dump: all feedback (open/awaiting_approval) with threads + all ai_proposals with messages
...
// Create ai_proposal
router.post('/proposals', checkSecret, asyncHandler(async (req, res) => {
...
// Update ai_proposal status (done/approved/rejected/revision)
router.patch('/proposals/:id', checkSecret, asyncHandler(async (req, res) => {
  const id = Number(req.params.id);
  ...
  res.json(await db.get('SELECT id, status, admin_notes FROM ai_proposals WHERE id = ?', id));
}));

// Post message to ai_proposal thread
router.post('/proposals/:id/message', checkSecret, asyncHandler(async (req, res) => {
  const id = Number(req.params.id);
  const { text } = req.body || {};
  if (!text) return res.status(400).json({ error: 'text required' });
  const adminUser = await db.get(`SELECT id FROM users WHERE role='admin' ORDER BY id LIMIT 1`);
