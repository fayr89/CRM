// TEMP — ежедневный диаг-обход AI (2026-06-10-v3). Снести сразу после использования.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'ai-daily-10jun2026-v3-k7q9';
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (action === 'patch_feedback') {
      const cur = await db.get('SELECT status FROM feedback WHERE id = ?', Number(body.id));
      ...
      await db.run(
        ...
        newStatus, body.admin_reply || null, Number(body.id)
      );
      return res.json({ ok: true });
    }
