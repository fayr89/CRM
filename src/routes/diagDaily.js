// TEMP: diag-эндпоинт для AI-обхода 2026-06-01 (сессия BvYtz).
// УДАЛИТЬ после использования.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'ai-dg7Yp3kXqN2wLmR8-BvYtz';
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    if (action === 'patch_proposal') {
      ...
      return res.json(await db.get('SELECT * FROM ai_proposals WHERE id = $1', id));
    }
    ...
    if (action === 'post_proposal') {
      ...
      return res.status(201).json(await db.get('SELECT * FROM ai_proposals WHERE id = $1', r.lastInsertRowid));
    }
    ...
    if (action === 'post_message') {
      ...
      return res.json(await db.get('SELECT * FROM feedback_messages WHERE id = $1', r.lastInsertRowid));
    }
    ...
    if (action === 'patch_feedback') {
      ...
      await db.run(
        `UPDATE feedback SET
           ...
           updated_at = NOW()
         WHERE id = $5`,
        newStatus, admin_reply, justResolved, adminUser?.id ?? null, id,
      );
