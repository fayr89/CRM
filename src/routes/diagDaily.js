// TEMP daily-run diagnostic endpoint — remove after use (2026-06-04-v10)
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'daily-20260604-v10-z7k9';
...
// GET /api/diag/daily?s=SECRET — все данные для ежедневного обхода
...
// GET /api/diag/daily/act?s=SECRET&a=ACTION&... — выполнить действие
router.get('/act', async (req, res) => {
  if (!guard(req, res)) return;
  const { a } = req.query;
  try {
    // Добавить сообщение в тред: &fid=X&text=...
    if (a === 'msg') {
    ...
    // Обновить статус/ответ обращения: &fid=X&status=...&reply=...
    if (a === 'upd-fb') {
      const fid = Number(req.query.fid);
    ...
    // Создать ai_proposal: &title=...&summary=...&risk=low&cat=...&fid=X
    if (a === 'create-prop') {
    ...
    // Обновить статус ai_proposal: &id=X&status=done|rejected|...
    if (a === 'upd-prop') {
