...
const SECRET = 'ai-daily-2026-06-03-Wm8v-J5zN3-v2';
...
// Читаем все обращения + треды + все ai_proposals по статусам
...
// op=close-fb         fb_id, msg (опционально)
// op=create-proposal  title, summary, category, risk, source, feedback_id, proposed_changes
// op=done-proposal    proposal_id
// op=resubmit-proposal proposal_id, summary (переработать и вернуть в pending)
router.get('/op', asyncHandler(async (req, res) => {
  ...
  if (op === 'create-proposal') {
    ...
    const changesJson = proposed_changes || null;
    const r = await db.run(
  ...
  if (op === 'resubmit-proposal') {
    const { proposal_id, summary } = req.query;
    if (!proposal_id) return res.status(400).json({ error: 'proposal_id required' });
    const sets = ["status = 'pending'", 'updated_at = NOW()'];
    const params = [];
    if (summary) { sets.push('summary = ?'); params.push(summary); }
    params.push(Number(proposal_id));
