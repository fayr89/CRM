// Временный диагностический эндпоинт для ежедневного обхода AI (v679).
...
router.get('/daily-v679', async (req, res) => {
  ...
  try {
      ...
      const rows = await db.all(
        `SELECT f.id, f.status, f.updated_at,
                (SELECT m.user_name FROM feedback_messages m WHERE m.feedback_id = f.id
                 ORDER BY m.created_at DESC, m.id DESC LIMIT 1) AS last_msg_user,
                (SELECT m.created_at FROM feedback_messages m WHERE m.feedback_id = f.id
                 ORDER BY m.created_at DESC, m.id DESC LIMIT 1) AS last_msg_at
         FROM feedback f
    ...
    if (op === 'create-proposal') {
      ...
      const source = req.query.source ? String(req.query.source) : 'daily-run-2026-07-29';
    ...
    return res.status(400).json({
      error: 'unknown op',
      ops: ['meta', 'get-feedback-summary', 'list-feedback', 'feedback-thread', 'post-feedback-message',
