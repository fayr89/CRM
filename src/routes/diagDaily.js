// Временный diag-эндпоинт для ежедневного обхода AI (v1125). Только GET, только
...
const router = Router();
const SECRET = 'daily-v1125-885429ec5c368cee41603056ff05c41f';
  ...
  asyncHandler(async (req, res) => {
    ...
    if (op === 'list-proposals') {
      ...
      const rows = await db.all(
        status
          ? `SELECT id, feedback_id, title, status, risk, updated_at, admin_decision_by, admin_decision_at, created_at FROM ai_proposals WHERE status = ? ORDER BY updated_at DESC`
          : `SELECT id, feedback_id, title, status, risk, updated_at, admin_decision_by, admin_decision_at, created_at FROM ai_proposals ORDER BY updated_at DESC`,
        ...(status ? [status] : []),
      );
    ...
    if (op === 'get-proposal') {
      const row = await db.get(
        `SELECT id, feedback_id, title, summary, category, risk, status, admin_notes, admin_decision_at
