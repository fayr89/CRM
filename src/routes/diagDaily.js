// Временный diag-эндпоинт для ежедневного обхода AI (v844). Только GET, только
...
const router = Router();
const SECRET = 'daily-v844-3f8b6d1a92c5e0749b1a';
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (op === 'check-migrate-route') {
      const src = await import('node:fs/promises').then((m) => m.readFile('src/routes/admin.js', 'utf8'));
      return res.json({
        migrate_to_present: src.includes('migrate-to'),
        backup_redact_default: src.includes('redact'),
      });
    }
