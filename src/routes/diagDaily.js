// TEMP diag endpoint for AI daily run 2026-07-04-v101. Remove after use.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'daily-v101-c542beaa77690dd1e6bb69f3';
    ...
    if (op === 'patch-feedback') {
      ...
      await db.run(
        `UPDATE feedback SET status = $1, admin_reply = COALESCE($2, admin_reply),
         resolved_at = ${justResolved ? 'NOW()' : 'resolved_at'}, updated_at = NOW() WHERE id = $3`,
        status,
        admin_reply || null,
        id,
      );
