...
// GET /api/diag/daily-v17?secret=... — proposals + open feedback with threads
...
// POST /api/diag/daily-v17/mark-done?secret=... — { id }
...
// POST /api/diag/daily-v17/feedback-patch?secret=... — { id, status, admin_reply? }
...
// POST /api/diag/daily-v17/post-message?secret=... — { feedback_id, text }
...
// POST /api/diag/daily-v17/create-proposal?secret=... — proposal fields
...
// POST /api/diag/daily-v17/close-feedback?secret=... — { id, message? }
router.post('/close-feedback', checkSecret, asyncHandler(async (req, res) => {
  ...
  await db.run(
    `UPDATE feedback SET status='closed', resolved_at=NOW(), updated_at=NOW() WHERE id=?`,
    id,
  );
  res.json({ ok: true, id });
}));
