// TEMP diag endpoint for AI daily run 2026-07-02-v55. Remove after use.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'daily-v55-Xr4mQp9zLwKfT2';
  ...
  try {
    ...
    if (op === 'meta') {
      const counts = await db.all(
        `SELECT 'feedback_open' AS k, COUNT(*)::int AS n FROM feedback WHERE status = 'open'
         UNION ALL SELECT 'feedback_awaiting', COUNT(*)::int FROM feedback WHERE status = 'awaiting_approval'
         UNION ALL SELECT 'proposals_pending', COUNT(*)::int FROM ai_proposals WHERE status = 'pending'
         UNION ALL SELECT 'proposals_approved', COUNT(*)::int FROM ai_proposals WHERE status = 'approved'
         UNION ALL SELECT 'proposals_revision', COUNT(*)::int FROM ai_proposals WHERE status = 'revision'
         UNION ALL SELECT 'proposals_rejected', COUNT(*)::int FROM ai_proposals WHERE status = 'rejected'`
      );
    ...
    res.json({
      ok: false,
      error: 'unknown op',
      ops: ['meta', 'get-proposals', 'get-proposal-messages', 'get-feedback', 'patch-proposal', 'post-proposal', 'post-proposal-msg', 'post-feedback-msg', 'patch-feedback'],
    });
  } catch (e) {
