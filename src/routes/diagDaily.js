// TEMP diag endpoint for AI daily run 2026-07-03-v86. Remove after use.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'daily-v86-9kQzT2mNc7Rf4WbXpLhY6sVd';
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    if (op === 'meta') {
      const branches = await db.get(`SELECT 1 AS ok`);
      return res.json({ ok: true, data: { db: !!branches } });
    }

    res.json({
      ok: false,
      error: 'unknown op',
      ops: ['get-proposals', 'get-proposal-messages', 'get-feedback', 'patch-proposal', 'post-proposal', 'post-proposal-msg', 'post-feedback-msg', 'patch-feedback', 'meta'],
    });
  } catch (e) {
