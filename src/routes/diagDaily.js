// TEMP diag endpoint for AI daily run 2026-07-12-v283. Remove after use.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'daily-v283-7f3ac9de1b2440e0a6c8e9d5f7b3a210';
    ...
    if (op === 'meta') {
      const branches = await db.get(
        `SELECT
           ...
           (SELECT COUNT(*) FROM feedback WHERE status = 'open') AS feedback_open_count
        `,
      );
