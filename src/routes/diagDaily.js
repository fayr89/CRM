// TEMP — daily AI feedback run diag route 2026-06-25-v4. Remove after use.
// Secret: mX7vQ2nLpW9kRt4s
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'mX7vQ2nLpW9kRt4s';
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    if (action === 'create-proposal') {
      ...
      const source = req.query.source || 'daily-run-2026-06-25-v4';
    ...
    // --- UPDATE AI PROPOSAL CONTENT (for revision) ---
    if (action === 'update-proposal') {
      const id = Number(req.query.id);
      const title = req.query.title ? b64(req.query.title) : null;
      const summary = req.query.summary ? b64(req.query.summary) : null;
      if (!id) return res.status(400).json({ error: 'id required' });
      const ups = ['updated_at = NOW()', "status = 'pending'"];
      const ps = [];
      if (title) { ups.push('title = ?'); ps.push(title); }
      if (summary) { ups.push('summary = ?'); ps.push(summary); }
      ps.push(id);
      await db.run(`UPDATE ai_proposals SET ${ups.join(', ')} WHERE id = ?`, ...ps);
      return res.json({ ok: true });
    }
