...
const router = Router();
const SECRET = 'daily-v1306-3e8b6f0a15dc';
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    if (op === 'search-feedback') {
      // поиск по тексту сообщений (для связки диагностических находок с обращениями)
      const q = `%${req.query.q || ''}%`;
      const rows = await db.all(
    ...
    if (op === 'proposal-create') {
      ...
      const source = req.query.source || 'daily-run-2026-08-25';
