// TEMP diag endpoint for AI daily run 2026-06-13-s21. Remove after use.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'daily-s21-pQ7mKz3r';
  ...
  try {
    ...
    if (op === 'get-proposal-messages') {
      ...
      const rows = await db.all(
        `SELECT id, proposal_id, user_name, text, created_at
    ...
    if (op === 'post-proposal') {
      ...
      const proposed_changes = proposed_changes_raw
        ? (typeof proposed_changes_raw === 'string'
            ? proposed_changes_raw
            : JSON.stringify(proposed_changes_raw))
        : null;
      ...
      return res.json({ ok: true, data: await db.get('SELECT * FROM ai_proposals WHERE id = $1', r.id) });
    }
    ...
    if (op === 'post-proposal-message') {
      ...
      const r = await db.get(
        `INSERT INTO ai_proposal_messages (proposal_id, user_name, text)
         VALUES ($1, 'AI ассистент', $2) RETURNING id`,
        id,
        text,
      );
    ...
    res.json({
      ok: false,
      error: 'unknown op',
      ops: ['get-proposals', 'get-proposal-messages', 'get-feedback', 'patch-proposal', 'post-proposal', 'post-proposal-message', 'post-feedback-msg', 'patch-feedback'],
    });
  } catch (e) {
