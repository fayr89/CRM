// TEMP: ежедневный AI-обход 2026-06-16-v33. Удалить сразу после обхода.
import { Router } from 'express';
import { db } from '../db.js';
import { asyncHandler } from '../errors.js';

const router = Router();
const TOKEN = 'aQ3rPm8xZk2vNbTy'; // одноразовый v33
...
router.get('/', asyncHandler(async (req, res) => {
  ...
  if (action === 'proposal_revision') {
    const id = Number(req.query.id);
    const summary = req.query.summary;
    if (!id || !summary) return res.status(400).json({ error: 'id,summary required' });
    ...
    await db.run(
      `UPDATE ai_proposals SET status='pending', summary=?, proposed_changes=COALESCE(?::jsonb, proposed_changes), updated_at=NOW() WHERE id=?`,
      summary, changesJson, id,
    );
