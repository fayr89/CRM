// TEMP diag endpoint for AI daily run 2026-06-10-v7. Remove after use.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'daily-v7-9xKp2mWq';
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    // ------- WRITE: mark proposal done/status -------
    ...
    // ------- WRITE: post message to ai_proposal thread -------
    if (op === 'post-proposal-msg') {
      ...
      if (!prop) return res.json({ ok: false, error: 'proposal not found' });
      const r = await db.run(
      ...
      return res.json({ ok: true, data: await db.get('SELECT id, user_name, role, text, created_at FROM ai_proposal_messages WHERE id = ?', r.lastInsertRowid) });
    }

    // ------- WRITE: post thread message to feedback -------
    if (op === 'post-feedback-msg') {
      ...
      return res.json({ ok: true, data: await db.get('SELECT id, user_name, role, text, created_at FROM feedback_messages WHERE id = ?', r.lastInsertRowid) });
    }
    ...
    res.json({ ok: false, error: 'unknown op', ops: ['get-proposals', 'get-proposal-messages', 'get-feedback', 'patch-proposal', 'post-proposal', 'post-proposal-msg', 'post-feedback-msg', 'patch-feedback'] });
  } catch (e) {
