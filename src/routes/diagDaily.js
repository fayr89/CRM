// TEMP diag endpoint for AI daily run 2026-07-03-v91. Remove after use.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'daily-v91-4YxWxLhmYqsN';
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    // ------- READ: count orphan dev branches (sanity check on runaway schedule) -------
    if (op === 'ping') {
    ...
    res.json({
      ok: false,
      error: 'unknown op',
      ops: ['get-proposals', 'get-proposal-messages', 'get-feedback', 'patch-proposal', 'post-proposal', 'post-proposal-msg', 'post-feedback-msg', 'patch-feedback', 'ping'],
    });
  } catch (e) {
