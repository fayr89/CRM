// ВРЕМЕННЫЙ diag-роут для ежедневного AI-обхода (daily-v1119). Анонимный
// (без auth) — доступ только по секрету в query. Удалить сразу после обхода.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'v1119-af20d3ab2d7827a0ef32';

router.get('/daily-v1119', async (req, res) => {
...
router.post('/daily-v1119', async (req, res) => {
  if (req.query.key !== SECRET) return res.status(404).json({ error: 'not found' });
