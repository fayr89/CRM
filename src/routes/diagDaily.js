// TEMP: ежедневный AI-обход 2026-06-06-v58. Снести сразу после использования.
import { Router } from 'express';
import { db } from '../db.js';

const SECRET = 'ai-daily-r8m4p2v6';
const router = Router();
    ...
    if (action === 'data') {
      ...
      for (const fb of feedbacks) {
        fb.messages = await db.all(
          'SELECT id,user_id,user_name,role,text,created_at FROM feedback_messages WHERE feedback_id = ? ORDER BY created_at ASC', fb.id);
      }
      ...
      const justResolved = needResolve && cur && !['awaiting_approval','closed'].includes(cur.status);
      await db.run(
