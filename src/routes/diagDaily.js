...
const router = Router();
const SECRET = 'daily-run-2026-06-07-a9f3';
...
router.get('/daily-data', async (req, res) => {
  ...
  try {
    // ai_proposals по всем статусам кроме done
    ...
    // Треды сообщений для каждого предложения
    ...
    // Группируем сообщения по proposal_id
    ...
    // Feedback — только open и awaiting_approval
...
router.get('/daily-write', async (req, res) => {
  ...
  try {
    // op=patch_proposal: { id, decision, notes }
    ...
    // op=post_proposal_msg: { id, text, user_name }
    ...
    // op=post_proposal: { title, summary, category, risk, source, proposed_changes, feedback_id }
    if (op === 'post_proposal') {
    ...
    // op=post_feedback_msg: { feedback_id, text, user_name }
    ...
    // op=patch_feedback: { id, status, admin_reply }
