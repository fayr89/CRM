// ВРЕМЕННЫЙ диагностический роут для ежедневного AI-обхода (сессия BInNe).
// Удалить сразу после использования отдельным коммитом.
import { Router } from 'express';
import { db } from '../db.js';

const SECRET = 'dRun-BInNe-2026-06-01-hJ5mKw';
  ...
  try {
    ...
    if (action === 'read_all') {
      ...
      const pending = await db.all(
        `SELECT * FROM ai_proposals WHERE status = 'pending' ORDER BY created_at DESC`,
      );
      return res.json({ feedback: feedbacks, proposals: { approved, revision, rejected, pending } });
    }
    ...
    if (action === 'update_proposal') {
      ...
      await db.run(
        ...
        decision, notes ?? null, adminId, id,
      );
      return res.json({ ok: true });
    }
