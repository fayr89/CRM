// TEMP: daily-run diag endpoint (сессия OEr0M). Удалить после обхода.
import { Router } from 'express';
import { db } from '../db.js';

const router = Router();
const SECRET = 'nK3mPq8sRv2wXc7hYb4tLj6dAe1oZf9u';
...
// GET /api/diag/daily-OEr0M?secret=...&action=...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    if (action === 'post-proposal') {
      ...
      if (!raw) return res.status(400).json({ error: 'data required (base64url JSON)' });
